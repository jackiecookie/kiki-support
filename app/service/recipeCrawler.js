...
module.exports = app => {
  class RecipeCrawler extends app.Service {
    errorProsses(error) {
    ...
    * findRecipes(mixinObj) {
      ...
      const promiseArray = yield new Promise(function(resolve) {
        ...
        const fn = co.wrap(function* (i, elm) {
          ...
          if ((!exist && rUrl && !recipeIdArr[recipeId]) || promiseArray.length === 0) {
            recipeIdArr[recipeId] = true;
    ...
    * recipePageCrawler(mixinObj) {
      ...
      if (!exist) {
        ...
        // 食谱描述图片
        const recipeImageUrlElm = $('#site-body .recipe-cover mip-img');
        const recipeImageUrl = hasElm(recipeImageUrlElm) ? recipeImageUrlElm[0].attribs.src : '';
        // 佐料
